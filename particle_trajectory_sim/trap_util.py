...
class trap:
	...
	def E_field(self, x, y, t):
	    """return the electric field at position (x, y) at time t; the x, y coordinates
	...
	def acceleration(self, x, y, t):
		"""return the acceleration of the particle at position (x, y) at time t;
	...
	def within_boundary(self, x, y):
	    """check whether the x, y, z coordinate is within our area of interest
	...
	def hit_electrodes(self, x, y, E_threshold=1.0e-7):
		"""check whether a particle hits an electrode if it is at position (x, y);
...
class quarter_trap(trap):
	...
	def hit_electrodes(self, x, y):
		...
		x, y = abs(x), abs(y)
		n = self.get_row_index(x, y)
		return (abs(self.df.iloc[n, 3]) <= 1.0e-7 and\
